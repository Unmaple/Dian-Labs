...
class DQNReplayer:
    def __init__(self, capacity):
        ...
        # 保存近1e5次的判断和对应结果，奖励（lunar也存了最近1e5次step）
        self.i = 0
        self.count = 0
    ...
    def store(self, *args):
        ...
        self.i = (self.i + 1) % self.capacity  # 保存最新一位所在位置（+1）
        self.count = min(self.count + 1, self.capacity)  #目前有的menmory总数

    def sample(self, size):
        indices = np.random.choice(self.count, size=size)
        out = (np.stack(self.memory.loc[indices, field]) for field in
                self.memory.columns)  # 将数组合并
        return out
...
class CategoricalDQNAgent:
    ...
    def learn(self):
        ...
        next_logit_tensor = self.target_net(next_state_tensor).view(-1,
                                                                    self.action_n, self.atom_count)  # tensor(32,6,51)
        next_prob_tensor = next_logit_tensor.softmax(dim=-1)  # 输出带概率的估计结果,格式如上
        next_q_tensor = (next_prob_tensor * self.atom_tensor).sum(2)  # （32,6)
        next_action_tensor = next_q_tensor.argmax(dim=1)  # 输出action选择的tensor（32,)
        next_actions = next_action_tensor.detach().numpy()  # 阻断反向梯度传播,np(32,
        next_dist_tensor = next_prob_tensor[np.arange(batch_size),next_actions, :].unsqueeze(1)  # (32,1,51)，？？？
        ...
        clipped_target_tensor = target_tensor.clamp(self.atom_min,
                                                    self.atom_max)  # 将超出范围的值减小到范围内（直接赋值上下界）
